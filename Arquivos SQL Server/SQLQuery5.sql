...
BEGIN
	...
	/* EXEMPLO */
	SELECT @V_CONT_RENAULT = COUNT(*) FROM TB_CARROS WHERE FABRICANTE = 'RENAULT'
	PRINT 'NUMEROS DE RENAULT: '+ CONVERT(VARCHAR, @V_CONT_RENAULT)

	/* IF DE ESTOQUE MARCA FIAT */
	IF  @V_CONT_FIAT > 0
		PRINT 'ESTOQUE DE CARROS FIAT: '+CONVERT(VARCHAR, @V_CONT_FIAT)
	ELSE
		PRINT 'ESTOQUE VAZIO DE FIATS'
END
GO
...
/* IF ELSE COM PROCEDURE */
CREATE TABLE TB_VERIFICA (
	NUMERO INT NOT NULL
);

DROP PROC PROC_VERIFIQUE_NUM
GO

CREATE PROC PROC_VERIFIQUE_NUM
	@NUMERO INT
AS
	INSERT INTO TB_VERIFICA VALUES (@NUMERO)

	PRINT 'VALOR INSERIDO COM SUCESSO: '+ CAST(@NUMERO AS VARCHAR)
...
BEGIN
	...
	CASE 
	    ...
	    WHEN FABRICANTE = 'RENAULT' THEN 'FAIXA 3'
		WHEN FABRICANTE = 'FORD' THEN 'FAIXA 4'  		
		ELSE 'OUTRAS FAIXAS'
	END AS MARCAS,
	*
	FROM TB_CARROS
...
BEGIN
	WHILE(@NUM <= 15)
	BEGIN
		PRINT 'VALOR DE @NUM = '+CAST(@NUM AS VARCHAR)
		SET @NUM = @NUM+1
	END
END
GO